...
/**
 * Update site name to ZeroEntropy News (runs once)
 */
function update_site_name_to_zeroentropy() {
	$site_name_updated = get_option( 'site_name_updated_to_zeroentropy', false );
	if ( ! $site_name_updated ) {
		update_option( 'blogname', 'ZeroEntropy News' );
		update_option( 'site_name_updated_to_zeroentropy', true );
	}
}
add_action( 'init', 'update_site_name_to_zeroentropy' );